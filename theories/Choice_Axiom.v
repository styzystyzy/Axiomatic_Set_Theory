...
Lemma Ex_bel_dom_set_res : forall x c,
...
Proof.
  intros; rewrite <- dom_ran_inv, <- dom_ran_inv'; auto.
Qed.

Lemma bel_dom_set_res : forall f g y,
  y ∈ dom(f) -> f [y] = g [f | (y)] -> Ensemble (f | (y)).
Proof.
  intros.
  generalize (classic ((f|(y)) ∈ dom(g))); intros; destruct H1; Ens.
  apply dom_value in H1; rewrite H1 in H0; clear H1.
  apply dom_value in H; rewrite H0 in *.
  generalize (notin_fix μ); intros; contradiction.
Qed.

Theorem exist_fun1_ordnum : forall x,
  Ensemble x -> exists f, Function1_1 f /\ ran(f) = x /\ Ordinal_Number dom(f).
Proof.
  intros.
  generalize Axiom_Choice; intros; destruct H0 as [c H0], H0.
  double H0; apply (Ex_bel_dom_set_res x _) in H2; auto; destruct H2 as [g H2].
  generalize (fun_ord_dom_value_res_eq g); intros; destruct H3 as [f H3], H3, H4.
  ...
  { unfold Function1_1; split; auto.
    ...
    - unfold Inverse in H7; destruct H7.
      ...
      { split; apply (ord_bel_ord dom(f) _); auto. }
      elim H12; intros; apply ord_bel_eq in H12.
      ...
      rewrite H2 in H13, H14; try apply (bel_dom_set_res _ g _); auto.
      ...
      + assert (f[z] ∈ ran(f|(z))).
        { rewrite H7; unfold Range; apply Axiom_Scheme; split; Ens.
          exists y; unfold Restriction; apply bel_inter; split; auto.
          unfold Cartesian; apply Axiom_SchemeP; split; Ens.
          split; auto; apply bel_universe_set; Ens. }
        assert ((x ~ ran(f|(z))) ∈ dom(c)).
        { generalize (classic ((x ~ ran(f|(z))) ∈ dom(c))); intros.
          destruct H16; auto; apply dom_value in H16; auto.
        ...
        rewrite <- H14 in H16; apply bel_inter in H16; destruct H16.
      ...
      + destruct H12; try contradiction.
        assert (f[y] ∈ ran(f|(y))).
        { rewrite <- H7; unfold Range; apply Axiom_Scheme; split; Ens.
          exists z; unfold Restriction; apply bel_inter; split; auto.
          unfold Cartesian; apply Axiom_SchemeP; split; Ens.
          split; auto; apply bel_universe_set; Ens. }
        ...
        { generalize (classic ((x ~ ran(f|(y))) ∈ dom(c))); intros.
          ...
          generalize universe_notset; intros; contradiction. }
        apply H6 in H16; unfold Difference at 2 in H16.
        rewrite <- H13 in H16; apply bel_inter in H16; destruct H16.
        unfold Complement in H17; apply Axiom_Scheme in H17; destruct H17.
        unfold NotIn in H18; contradiction. }
  ...
  { unfold Subclass; intros; unfold Range in H8; apply Axiom_Scheme in H8.
    ...
    { unfold Ordinal_Number, R; apply Axiom_Scheme; split; Ens.
      apply (ord_bel_ord dom(f) _); split; auto. }
    apply H5 in H11; rewrite H2 in H11; try apply (bel_dom_set_res _ g _); auto.
    ...
    { generalize (classic ((x ~ ran(f|(x0))) ∈ dom(c))); intros.
      destruct H13; auto; apply dom_value in H13; auto.
  ...
  { unfold Function1_1 in H7; destruct H7 as [H9 H7]; clear H9.
    ...
    rewrite <- H10; apply Axiom_Substitution; apply sub_set in H8; auto. }
  ...
  { unfold Restriction; apply Axiom_Extent; split; intros.
    ...
    - apply Axiom_Scheme; repeat split; Ens; unfold Function, Relation in H3.
      ...
      + apply Property_ran in H11; apply bel_universe_set; Ens. }
  rewrite H11 in *; clear H11.
  rewrite H2 in H10; try apply fun_dom_set; auto.
  generalize (notin_fix dom(f)); intros.
  apply dom_value in H11; auto; rewrite H10 in H11.
  generalize (classic ((x ~ ran(f)) ∈ dom(c))); intros; destruct H12.
  - apply dom_value in H12; auto; rewrite H11 in H12.
    generalize (notin_fix μ); intros; contradiction.
  - rewrite H1 in H12; unfold Difference at 2 in H12.
    assert ((x ~ ran(f)) ∈ (μ ∩ ¬[Φ]) <-> (x ~ ran(f)) ∈ μ /\
            (x ~ ran(f)) ∈ ¬[Φ]).
    { split; intros; try apply bel_inter; auto. }
    apply definition_not in H13; auto; clear H12.
    assert (Ensemble (x ~ ran(f))).
    { apply (sub_set x _); auto; unfold Subclass.
      intros; apply Axiom_Scheme in H12; apply H12. }
    apply not_and_or in H13; destruct H13.
    + elim H13; apply bel_universe_set; auto.
    + assert ((x ~ ran(f)) ∈ ¬[Φ] <-> Ensemble (x ~ ran(f)) /\
      ...
      AssE Φ; clear H15 H16 H17; apply bel_universe_set in H18.
      apply H14 in H18; symmetry; apply -> Property_Φ in H18; auto.
Qed.

Hint Resolve exist_fun1_ordnum : set.
...
Lemma lem_Hausdorff_maximal_principle : forall f z,
...
Proof.
  ...
  double H3; rewrite fun_set_eq in H4; auto.
  apply Axiom_SchemeP in H4; destruct H4; rewrite H5 in *.
  apply Property_dom in H3; Ens.
Qed.

Theorem Hausdorff_maximal_principle : forall x,
...
Proof.
  ...
  { intros; assert (Ordinal_Number u).
    { unfold Ordinal_Number, R; apply Axiom_Scheme.
      split; try apply (ord_bel_ord dom(f) _); Ens. }
    apply H5 in H8; rewrite H2 in H8; try apply (bel_dom_set_res _ g _); auto.
    assert (En_c x (f|(u)) ∈ dom(c)).
    { generalize (classic (En_c x (f|(u)) ∈ dom(c))); intros.
      destruct H9; auto; apply dom_value in H9; auto.
  ...
  { intros; apply H7 in H9; clear H7; destruct H9, H9.
    ...
    unfold Cartesian; apply Axiom_SchemeP; split; Ens.
    split; try apply bel_universe_set; Ens. }
  exists (∪ ran(f)); intros; split; intros; try split.
  - unfold Nest; intros z0 z1 H9; destruct H9.
    apply lem_Hausdorff_maximal_principle in H9; apply lem_Hausdorff_maximal_principle in H10; auto.
    destruct H9, H10, H9, H10.
    assert (Ordinal x0 /\ Ordinal x1).
    { split; apply (ord_bel_ord dom(f) _); auto. }
    apply ord_bel_eq in H13; destruct H13 as [H13|[H13|H13]].
    ...
    + rewrite H13 in H9; apply H7 in H12; destruct H12.
      unfold Nest in H12; apply H12; auto.
  - unfold Subclass; intros; apply lem_Hausdorff_maximal_principle in H9; auto.
    destruct H9, H9; apply H7 in H10; destruct H10, H11.
    unfold Subclass in H11; apply H11; auto.
  - destruct H10; generalize (notin_fix dom(f)); intros.
    apply dom_value in H12.
    assert (Function f⁻¹).
    { unfold Function; split; intros.
      ...
      - destruct H13; unfold Inverse in H13, H14.
        ...
        rewrite fun_set_eq in H17, H18; auto.
        ...
        assert (Ordinal y /\ Ordinal z).
        { split; apply (ord_bel_ord dom(f) _); auto. }
        apply ord_bel_eq in H17; destruct H17.
    ...
    { generalize (Property_F11 f); intros; destruct H14.
      ...
      apply (sub_set pow(x) _); try (apply pow_set in H; auto).
      unfold Subclass; intros; unfold Range in H16; apply Axiom_Scheme in H16.
      destruct H16, H17; double H17; rewrite fun_set_eq in H18; auto.
    ...
    { unfold Restriction; apply Axiom_Extent; split; intros.
      ...
      - apply Axiom_Scheme; repeat split; Ens; unfold Function, Relation in H3.
        ...
        + apply Property_ran in H16; apply bel_universe_set; Ens. }
    rewrite H16 in *; rewrite H15 in H12; clear H15 H16.
    rewrite H2 in H12; try apply fun_dom_set; auto.
    generalize (classic (En_c x f ∈ dom(c))); intros; destruct H15.
    + apply dom_value in H15; auto; rewrite H12 in H15.
      generalize (notin_fix μ); intros; contradiction.
    + rewrite H1 in H15; unfold Difference in H15.
      ...
      * elim H16; apply bel_universe_set; auto.
      * assert (En_c x f ∈ ¬[Φ] <-> Ensemble (En_c x f) /\ En_c x f ∉ [Φ]).
        ...
        AssE Φ; clear H18 H19 H20; apply bel_universe_set in H21.
        ...
        { unfold En_c; apply Axiom_Scheme; repeat split; auto.
          - apply (sub_set x _); auto.
          - unfold Subclass; intros; apply H11.
            unfold Element_U; apply Axiom_Scheme; split; Ens.
          - intro; rewrite H19 in H18; clear H19.
            elim H17; apply sub_eq; split; auto.
            unfold Subclass; intros; apply Axiom_Scheme; Ens. }
        rewrite H21 in H18; generalize (not_bel_zero m); contradiction.
Qed.

Hint Resolve Hausdorff_maximal_principle : set.
