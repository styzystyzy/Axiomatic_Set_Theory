...
Definition Connect r x : Prop := forall u v, u∈x /\ v∈x -> (Rrelation u r v) \/ (Rrelation v r u) \/ (u=v).
...
Definition Asymmetric r x : Prop := forall u v, (u ∈ x /\ v ∈ x /\ Rrelation u r v) -> ~ Rrelation v r u.

Hint Unfold Asymmetric: set.

Corollary Property_Asy : forall r x u, Asymmetric r x -> u ∈ x -> ~ Rrelation u r u.
Proof.
  intros; intro.
...
Definition WellOrdered r x : Prop := Connect r x /\ (forall y, y⊂x /\ y≠Φ -> exists z, FirstMember z r y).
...
Theorem Theorem88 : forall r x, WellOrdered r x -> Transitive r x /\ Asymmetric r x .
Proof.
  ...
  { unfold Asymmetric; intros.
     ...
     { split.
        ...
        - apply Lemma35; exists u; apply AxiomII; split; auto;  left; apply AxiomII; split; auto. }
  ...
  * subst w; unfold Asymmetric in H2.
...
Definition Section y r x : Prop :=
  y ⊂ x /\ WellOrdered r x /\ (forall u v, (u ∈ x /\ v ∈ y /\ Rrelation u r v) -> u ∈ y).

(* 定理90 *)

Theorem Theorem90 : forall n x r,
  n ≠ Φ /\ (forall y, y ∈ n -> Section y r x) -> Section (∩ n) r x /\ Section (∪ n) r x.
Proof.
  intros; destruct H.
  double H; apply Lemma35 in H; destruct H; double H.
  ...
  - split; try split; auto; intros.
    + unfold Included; intros; apply AxiomII in H5; destruct H5, H6, H6.
        apply H0 in H7; red in H7; destruct H7 as [H7 _]; auto.
    + destruct H5, H6.
        apply AxiomII; split; intros; Ens.
        apply AxiomII in H6; destruct H6, H8, H8.
        double H9; apply H0 in H9; unfold Section in H9; destruct H9 as [_ [_ H9]].
        exists x1; split; auto; eapply H9; split; eauto.
Qed.
...
Theorem Theorem91 : forall x y r,
  Section y r x /\ y≠x -> (exists v, v ∈ x /\ y = \{ λ u, u ∈ x /\ Rrelation u r v \}). 
Proof.
  intros; destruct H.
  ...
  { unfold Section in H; destruct H, H1; unfold WellOrdered in H1; destruct H1.
     ...
     { red; intros; apply AxiomII in H4; tauto. }
      ...
      + apply Property_Φ in H; apply H in H5; apply Property_Ineq in H0; contradiction.
      + apply H3; split; auto. }
...
Proof.
  ...
  - generalize (classic (y = z)); intro; destruct H2.
    ...
    + apply Lemma_xy with (x:=(Section x r z)) in H1; auto.
       ...
       unfold Section in H; destruct H as [_ [H _]]; unfold WellOrdered in H; destruct H as [H _].
...
Theorem Theorem94 : forall x r y f,
  Section x r y /\ Order_Pr f r r /\ On f x /\ To f y -> (forall u, u ∈ x -> ~ Rrelation f[u] r u).
Proof.
  intros; destruct H, H1, H2.
  unfold Order_Pr in H1; destruct H1, H4, H5.
  unfold On in H2; destruct H2 as [H2 H7]; unfold To in H3; destruct H3 as [_ H3].
  generalize (classic (\{ λ u, u ∈ x /\ Rrelation (Value f u) r u \} = Φ)); intros; destruct H8.
  - intro.
    assert (u ∈ Φ).
  ...
  - unfold Section in H; destruct H, H9.
    ...
    apply Lemma_xy with (y:=(\{ λ u : Class,u ∈ x /\ Rrelation f [u] r u \} ≠ Φ)) in H11; auto.
    apply H12 in H11; destruct H11; unfold FirstMember in H11; destruct H11.
    apply AxiomII in H11; destruct H11, H14.
    assert (f[x0] ∈ ran( f)).
    { rewrite <- H7 in H14; apply Property_Value in H14; auto; apply Property_ran in H14; auto. }
...
Lemma Lemma96'' : forall f u, Function f -> Function f ⁻¹ -> u ∈ ran(f) ->  (f ⁻¹) [u] ∈ dom(f).
Proof.
...
Lemma Lemma96''' : forall f u, Function f -> Function f ⁻¹ -> u ∈ ran(f) -> u = f  [(f ⁻¹) [u]].
Proof.
...
Proof.
  ...
  { unfold Function1_1; split; auto.
    ...
    + destruct H3; rename y into u; rename z into v.
       ...
       rewrite Theorem70 in H9; auto; apply AxiomII_P in H9; destruct H9 as [_ H9].
       rewrite Theorem70 in H10; auto; apply AxiomII_P in H10; destruct H10 as [_ H10].
      ...
      unfold WellOrdered in H1; destruct H1 as [H1 _]; unfold Connect in H1; specialize H1 with f [u] f [v].
      unfold WellOrdered in H0; destruct H0; unfold Connect in H0; specialize H0 with u v.
      destruct H0 as [H0 | [H0 | H0]]; try split; auto.
      * assert (Rrelation f [u] s f [v]); try apply H2; try tauto.
         rewrite H9 in H14; generalize (Property_Asy _ _ _ H12 H8); intro; contradiction.
...
Lemma Lemma97' :  forall f g u r s v x y, 
  Order_Pr f r s /\ Order_Pr g r s -> FirstMember u r (\{ λ a ,a ∈ (dom( f) ∩ dom( g)) /\ f [a] ≠ g [a] \}) ->
  g[v] ∈ ran( g) -> Section ran( f) s y -> Section dom( f) r x -> Section dom( g) r x ->
  Rrelation g [v] s g [u] -> f[u] = g[v] -> f ⊂ g \/ g ⊂ f.
Proof.
  intros.
  ...
  { elim H11; intros; clear H13.
     ...
     apply Property_Value' in H10; auto; apply Property_dom in H10; rewrite Lemma96 in H10.
     apply Property_Value' in H1; auto; apply Property_dom in H1; rewrite Lemma96 in H1.
     rewrite Lemma96''' with (f:=(g ⁻¹)); try rewrite Theorem61; auto.
     pattern v; rewrite Lemma96''' with (f:=(g ⁻¹)); try rewrite Theorem61; auto. }
  ...
  { apply Property_Value' in H1; try tauto; apply Property_dom in H1.
     ...
     - intro.
       ...
       generalize (Lemma97 _ _ _ H16 H2); intro; apply Theorem88 in H18; destruct H18.
...
Proof.
  ...
  generalize (classic (\{ λ a ,a ∈ (dom( f) ∩ dom( g)) /\ f [a] ≠ g [a] \} = Φ)); intro; destruct H6.
  ...
  - assert (\{ λ a,a ∈ (dom( f) ∩ dom( g)) /\ f [a] ≠ g [a] \} ⊂ dom(f)).
    { unfold Included; intros; apply AxiomII in H7; destruct H7, H8; apply Theorem4' in H8; tauto. }
    ...
    * apply H23 in H17; double H17.
       ...
       rewrite Theorem70 in H17; auto; apply AxiomII_P in H17; destruct H17; rewrite H25 in H24.
       ...
       { red in H1; destruct H1 as [_ [_ H1]].
          rewrite <- H25 in H26.
          assert (g [u] ∈ ran( f)).
          { apply H1 with f [u]; repeat split; auto.
             unfold Section in H4; apply H4.
             apply Property_ran with u; apply Property_Value; auto.
             apply Property_ran with u; apply Property_Value; auto. }
           apply AxiomII in H27; destruct H27 as [_ [v1 H27]].
           double H27; apply Property_dom in H28; apply Property_Value in H28; auto.
           rewrite Theorem70 in H27; auto; apply AxiomII_P in H27; destruct H27 as [_H27].
          rewrite H27 in H26.
          assert (g ⊂ f \/ f ⊂ g).
          { apply (Lemma97' g f u r s v1 x y); try tauto; auto.
             rewrite Theorem6'.
             assert (\{ λ a : Class,a ∈ (dom( f) ∩ dom( g)) /\ f [a] ≠ g [a] \}= \{ λ a : Class,a ∈ (dom( f) ∩ dom( g)) /\ g [a] ≠ f [a] \}).
             { apply AxiomI; split; intros; apply AxiomII in H29; apply AxiomII; repeat split; try tauto.
                apply Property_Ineq; tauto.
                apply Property_Ineq; tauto. }
          rewrite <- H29; auto.
          apply Property_ran in H28; auto. }
        tauto. }
    ...
    * apply H23 in H20; double H18.
       ...
       apply Property_ran in H20.
       generalize (Lemma_xy _ _ H17 H20); intro.
       unfold WellOrdered in H11; destruct H11 as [H11 _]; apply H11 in H27.
       destruct H27 as [H27 | [H27 | H27]]; try contradiction.
       -- { unfold Section in H4; destruct H4 as [_ [_ H4]].
            assert (f[u] ∈ ran( g)).
            { apply H4 with g[u]; repeat split; auto.
               red in H1; apply H1.
               apply Property_ran with u; apply Property_Value; auto.
               apply Property_ran with u; apply Property_Value; auto. }
             ...
             rewrite H30 in H27.
            apply (Lemma97' f g u r s v1 x y); try tauto; auto.
            apply Property_ran with v1; auto. }
    -- assert (g ⊂ f \/ f ⊂ g).
        { rewrite <- H26 in H27, H20. apply (Lemma97' g f u r s v x y); try tauto; auto.
           rewrite Theorem6'.
           assert (\{ λ a : Class,a ∈ (dom( f) ∩ dom( g)) /\ f [a] ≠ g [a] \}= \{ λ a : Class,a ∈ (dom( f) ∩ dom( g)) /\ g [a] ≠ f [a] \}).
           { apply AxiomI; split; intros; apply AxiomII in H28; apply AxiomII; repeat split; try tauto.
              apply Property_Ineq; tauto.
              apply Property_Ineq; tauto. }
           rewrite <- H28; auto. }
           tauto.
...
Definition Order_PXY  f x y r s : Prop :=
  WellOrdered r x /\ WellOrdered s y /\ Order_Pr f r s /\ Section dom(f) r x /\ Section ran(f) s y.
...
Lemma Lemma99' : forall a b f z,
  ~ a ∈ dom(f) -> Ensemble a -> Ensemble b -> (z ∈ dom(f) -> (f ∪ [[a,b]]) [z] = f [z]).
Proof.
  intros; apply AxiomI; split; intros; apply AxiomII in H3; destruct H3; apply AxiomII; split; intros; auto.
  - apply H4; apply AxiomII in H5; destruct H5; apply AxiomII; split; auto; apply AxiomII; split; Ens.
  - apply H4; apply AxiomII in H5; destruct H5; apply AxiomII; split; auto.
    apply AxiomII in H6; destruct H6, H7; auto; apply AxiomII in H7; destruct H7.
    assert ([a, b] ∈ μ). { apply Theorem19; apply Theorem49; tauto. }
    generalize (H8 H9); intro; apply Theorem55 in H10; apply Theorem49 in H6; auto; destruct H10.
...
Proof.
  ...
  - apply AxiomII; split; intros; Ens; apply AxiomII in H2; destruct H2; apply AxiomII in H4; destruct H4, H5.
    + apply Property_dom in H5; contradiction.
    + apply AxiomII in H5; destruct H5.
        assert ([a, b] ∈ μ). { apply Theorem19; apply Theorem49; tauto. }
        generalize (H6 H7); intro;
        apply Theorem55 in H8; apply Theorem49 in H4; auto; destruct H8; rewrite H9; auto.
Qed.

Lemma Lemma99''' : forall y r x a b,  Section y r x -> a ∈ y -> ~ b ∈ y -> b ∈ x -> Rrelation a r b.
Proof.
...
Definition En_f x y r s := \{\ λ u v, u ∈ x /\ (exists g, Function g /\ Order_PXY g x y r s /\ u ∈ dom(g) /\ [u,v] ∈ g ) \}\.

Theorem Theorem99 : forall r s x y,
  WellOrdered r x /\ WellOrdered s y -> exists f, Function f /\ Order_PXY f x y r s /\((dom(f) = x) \/ (ran(f) = y)).
Proof.
  intros.
  assert (Function (En_f x y r s)).
  { unfold Function; split; intros.
     - unfold Relation; intros; PP H0; eauto.
     - destruct H0.
  ...
  { unfold Section; split.
     ...
     - split; try tauto; intros; destruct H1, H2; apply AxiomII in H2; destruct H2, H4.
     ...
     { destruct H6, H7; unfold Order_PXY in H7; destruct H7, H9, H10, H11.
       unfold Section in H11; destruct H11, H13; apply H14 with v; destruct H8; tauto. }
     exists (x1[u]).
     apply AxiomII_P; repeat split; auto.
     * apply Theorem49; split; Ens.
        apply Theorem19; apply Theorem69; try tauto.
     * exists x1; split; try tauto; split; try tauto; split; auto; apply Property_Value; try tauto. }
  assert (Section (ran(En_f x y r s)) s y).
    { unfold Section; split.
       - unfold Included; intros; apply AxiomII in H2; destruct H2, H3.
         ...
         unfold Order_PXY in H6; destruct H6 as [_ [_ [_ [_ H6]]]]; unfold Section in H6; destruct H6 as [H6 _].
         apply Property_ran in H8; auto.
       - split; try tauto; intros; destruct H2, H3; apply AxiomII in H3; destruct H3, H5.
         ...
         assert (u ∈ ran( x1)). { apply Property_ran in H10; apply H15 with v; tauto. }
         generalize H14 as H21; intro.
         apply Theorem96 in H12; destruct H12; unfold Function1_1 in H12; destruct H12.
  ...
  { unfold Order_PXY; split; try tauto; split; try tauto; split; [idtac | tauto].
     ...
     split; intros; try eapply Lemma99; eauto; destruct H4, H5; double H4; double H5.
     ...
     rewrite Theorem70 in H13; auto; apply AxiomII_P in H13; destruct H13 as [_ H13].
     rewrite Theorem70 in H18; auto; apply AxiomII_P in H18; destruct H18 as [_ H18].
     ...
     generalize (Lemma_xy _ _ H11 H16); intro; apply (Theorem97 g1 g2 r s x y) in H21; auto.
  ...
  { unfold Section in H1; destruct H1, H6.
     assert ((x ~ dom( En_f x y r s)) ⊂ x). { red; intros; apply AxiomII in H8; tauto. }
    assert ((x ~ dom( En_f x y r s)) <> Φ).
    { intro; apply Property_Φ in H1; apply H1 in H9; apply H4; auto. }
    generalize (Lemma97 _ _ _ H6 H8); intro; apply H10; repeat split; auto; red; auto. }
  assert (exists v, FirstMember v s (y ~ ran( En_f x y r s))).
  { unfold Section in H2; destruct H2, H7.
     assert ((y ~ ran( En_f x y r s)) ⊂ y). { red; intros; apply AxiomII in H9; tauto. }
    assert ((y ~ ran( En_f x y r s)) <> Φ).
    { intro; apply Property_Φ in H2; apply H2 in H10; apply H5; auto. }
    generalize (Lemma97 _ _ _ H7 H9); intro; apply H11; repeat split; auto; red; auto. }
  ...
  { assert ([u, v] ∈ μ) as H18. { apply Theorem19; apply Theorem49; split; try Ens. }
     unfold Function; split; intros.
     ...
     - destruct H11.
       apply AxiomII in H11; destruct H11 as [H11 [H13 | H13]]; 
       apply AxiomII in H12; destruct H12 as [H12 [H14 | H14]].
      ...
      * apply AxiomII in H13; destruct H13; apply Theorem55 in H15; apply Theorem49 in H13; auto.
         apply AxiomII in H14; destruct H14; apply Theorem55 in H16; apply Theorem49 in H12; auto.
  ...
  { unfold Section; split.
     - unfold Included; intros; apply AxiomII in H12; destruct H12, H13.
       ...
       * apply AxiomII in H14; destruct H14.
          assert ([u, v] ∈ μ). { apply Theorem19; apply Theorem49; split; try Ens. }
          apply H15 in H16; apply Theorem55 in H16; apply Theorem49 in H13; auto; destruct H16.
          rewrite H16; auto.
     - split; try tauto; intros; destruct H12, H13; apply AxiomII in H13; destruct H13, H15.
       apply AxiomII in H15; destruct H15, H16.
       * apply AxiomII; split; Ens.
          assert ([u0, (En_f x y r s) [u0]] ∈ (En_f x y r s)).
          { apply Property_dom in H16; apply Property_Value; auto; apply H1 with v0; repeat split; auto. }
          exists (En_f x y r s) [u0].
          apply AxiomII; split; Ens.
       * apply AxiomII in H16; destruct H16.
          assert ([u,v] ∈ μ). { apply Theorem19; apply Theorem49; split; Ens. }
          apply H17 in H18.
          apply Theorem55 in H18; apply Theorem49 in H16; auto; destruct H18.
          subst v0.
          assert ([u0, (En_f x y r s) [u0]] ∈ (En_f x y r s)).
          { apply Property_Value; auto; generalize (classic (u0 ∈ dom( En_f x y r s))); intro; destruct H18; auto.
             absurd (Rrelation u0 r u); auto.
             apply H8; apply AxiomII; repeat split; Ens; apply AxiomII; split; Ens. }
          apply AxiomII; split; Ens.
          exists ((En_f x y r s)[u0]); apply AxiomII.
          split; Ens. }
  ...
  { unfold Section; split.
     - unfold Included; intros; apply AxiomII in H13; destruct H13, H14.
       ...
       * apply AxiomII in H15; destruct H15.
          assert ([u, v] ∈ μ). { apply Theorem19; apply Theorem49; split; try Ens. }
          apply H16 in H17; apply Theorem55 in H17; apply Theorem49 in H14; auto; destruct H17.
          rewrite H18; auto.
     - split; try tauto; intros; destruct H13, H14; apply AxiomII in H14; destruct H14, H16.
       unfold Order_PXY in H3; destruct H3 as [_ [_ [H3 _]]]; apply Theorem96 in H3; destruct H3 as [[_ H3] _].
       apply AxiomII in H16; destruct H16, H17.
       * apply AxiomII; split; Ens.
          assert ([((En_f x y r s) ⁻¹) [u0], u0] ∈ (En_f x y r s)).
         { assert (u0 ∈ ran( En_f x y r s)). { apply Property_ran in H17; apply H2 with v0; repeat split; auto. }
            pattern u0 at 2; rewrite Lemma96''' with (f:=(En_f x y r s)); auto. apply Property_Value'; auto.
            rewrite <- Lemma96'''; auto. }
         exists ((En_f x y r s) ⁻¹) [u0]; apply AxiomII; split; Ens.
      * apply AxiomII in H17; destruct H17.
         assert ([u,v] ∈ μ). { apply Theorem19; apply Theorem49; split; Ens. }
         apply H18 in H19; apply Theorem55 in H19; apply Theorem49 in H16; auto; destruct H19.
          subst v0.
          assert ([((En_f x y r s) ⁻¹) [u0], u0] ∈ (En_f x y r s)).
          { generalize (classic (u0 ∈ ran( En_f x y r s))); intro; destruct H20.
             ...
             - absurd (Rrelation u0 s v); auto.
               apply H9; apply AxiomII; repeat split; Ens; apply AxiomII; split; Ens. }
          apply AxiomII; split; Ens.
          exists ((En_f x y r s) ⁻¹) [u0]; apply AxiomII; split; Ens. } 
  split.
  - unfold Order_PXY; split; try tauto; split; try tauto; split; [idtac | tauto].
    unfold Order_Pr; intros.
    split; auto.
    split; try eapply Lemma99; eauto; try apply H.
    split; try eapply Lemma99; eauto; try apply H; intros.
    destruct H14, H15.
    apply AxiomII in H14; destruct H14, H17; apply AxiomII in H17; destruct H17 as [_ H17].
    apply AxiomII in H15; destruct H15, H18; apply AxiomII in H18; destruct H18 as [_ H18].
    assert ([u,v] ∈ μ) as H20. { apply Theorem19; apply Theorem49; split; Ens. }
    destruct H17, H18.
    * apply Property_dom in H17; apply Property_dom in H18; repeat rewrite Lemma99'; auto; Ens.
    ...
    * apply Property_dom in H17; rewrite Lemma99'; auto; Ens.
       ...
       apply H19 in H20;  apply Theorem55 in H20; destruct H20; apply Theorem49 in H18; auto.
      rewrite Lemma99''; auto; Ens; apply Lemma99''' with (y:=(ran( En_f x y r s))) (x:=y); auto.
      + apply Property_Value in H17; auto; double H17; apply Property_ran in H17.
    ...
    * apply Property_dom in H18; pattern ((En_f x y r s ∪ [[u, v]]) [v0]); rewrite Lemma99'; Ens.
       ...
       unfold Order_Pr in H3; eapply H3; eauto.
    * double H20.
       apply AxiomII in H17; destruct H17; apply H21 in H19; apply AxiomII in H18; destruct H18; apply H22 in H20.
...
Proof.
  intros; destruct H.
  generalize (Lemma_xy _ _ H H2); intro; apply Theorem99 in H3; destruct H3, H3, H4.
