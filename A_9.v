  ...
  - assert ((En_S P) ⊂ W).
    ...
    + destruct H7; subst h.
      ...
      { apply AxiomII; repeat split; Ens.
        unfold W in H4; apply AxiomII in H4; clear H2; destruct H4.
        apply AxiomII; split; Ens; eapply Theorem132; eauto. }
...
Proof.
  split; intros.
  - apply AxiomII in H1; destruct H1, H3; apply AxiomI; split; intros.
    + apply H0 in H5; auto.
    + apply H with (n:= z) in H4; auto; try apply AxiomII; Ens; intros.
      destruct H6; assert (k ∈ \{λ x, x∈W /\ P x\}). { apply AxiomII; Ens. }
      apply H2 in H8; apply AxiomII in H8; apply H8.
  - assert (\{ λ x, x ∈ W /\ P x \} ⊂ W).
    { unfold Included; intros; apply AxiomII in H3; apply H3. }
    assert (Φ ∈ \{ λ x, x ∈ W /\ P x \}).
    { apply AxiomII; generalize (Theorem135 n); intros.
      destruct H4 as [H4 _]; Ens. }
    assert ((forall u, u ∈ \{ λ x, x ∈ W /\ P x \} ->
     (PlusOne u) ∈ \{ λ x, x ∈ W /\ P x \})).
    { intros; apply AxiomII in H5; destruct H5, H6; double H6.
      apply Theorem134 in H8; apply AxiomII; repeat split; Ens. }
