...
Axiom AxiomVII : forall x, x ≠ Φ -> exists y:Class, y ∈ x /\ x ∩ y = Φ.
...
Proof.
  ...
  assert (x0 = x). { apply AxiomII in H0; destruct H0; apply H2; apply Theorem19; Ens. }
  subst x0.
...
Proof.
  ...
  assert (\{ λ z, z = x \/ z =y \} ≠ Φ). { apply Lemma35; exists x; apply AxiomII; split; Ens. }
...
Proof.
  intro.
  generalize (Theorem42 _ H); intro.
  assert (E ∈ [E]).
...
Theorem Theorem107 : forall x, Ordinal x -> WellOrdered E x.
Proof.
  intros.
  unfold Ordinal in H; destruct H.
  unfold WellOrdered; intros.
...
Lemma Lemma109' : forall x y, Ordinal x /\ Ordinal y -> ((x ∩ y) = x) \/ ((x ∩ y) ∈ x).
Proof.
  intros; generalize (classic ((x ∩ y) = x)); intro.
  destruct H0; try tauto.
  - assert ((x ∩ y) ⊂ x). { unfold Included; intros; apply Theorem4' in H1; tauto. }
    elim H; intros; apply Lemma109 in H.
    eapply Theorem108 in H2; eauto.
Qed.

Theorem Theorem109 : forall x y, Ordinal x /\ Ordinal y -> x ⊂ y \/ y ⊂ x.
Proof.
  ...
  - destruct H2.
    ...
    + assert ((x ∩ y) ∈ (x ∩ y)). { rewrite Theorem6' in H2; apply AxiomII; repeat split; Ens. }
...
Theorem Theorem110 : forall x y, Ordinal x /\ Ordinal y -> x ∈ y \/ y ∈ x \/ x = y.
Proof.
  intros; generalize (classic (x = y)); intro; destruct H0; try tauto.
  elim H; intros; apply Theorem109 in H.
  destruct H.
...
Definition LessEqual x y := In x y \/ x=y.
...
Theorem Theorem118 : forall x y, Ordinal x /\ Ordinal y -> (x ⊂ y <-> x ≼ y).
Proof.
...
Theorem Theorem119 : forall x, Ordinal x -> x = \{ λ y, (In y R /\ Less y x) \}.
Proof.
...
Proof.
  ...
  - unfold Connect; intros; destruct H0.
    apply AxiomII in H0; destruct H0, H2, H2; apply AxiomII in H1; destruct H1, H4, H4.
...
Proof.
  ...
  - unfold Connect; split.
    * unfold Connect; intros; destruct H0.
       apply AxiomII in H0; destruct H0; apply AxiomII in H1; destruct H1.
       destruct H2, H3.
    ...
    * unfold full; intros; unfold Included; intros.
       ...
       -- unfold Ordinal in H3; destruct H3.
           generalize (Property_Full x); intro; destruct H5.
           apply H5 with (u:=z) (v:=m) in H4; tauto.
       -- apply AxiomII in H2; destruct H2.
           apply Theorem19 in H; apply H4 in H; subst m; tauto.
Qed.

Theorem Theorem123 : forall x, x ∈ R -> FirstMember (PlusOne x) E (\{ λ y, (In y R /\ Less x y) \}).
Proof.
  ...
  - intro; apply AxiomII in H0; destruct H0, H2.
    ...
    + AssE x; apply Theorem19 in H6; apply AxiomII in H5; destruct H5; apply H7 in H6.
       subst y; eapply Theorem101; eauto.
Qed.
...
Theorem Theorem124 : forall x, x ∈ R -> ∪ PlusOne x = x.
Proof.
  intros; apply AxiomI; split; intros.
  - apply AxiomII in H0; destruct H0, H1, H1; apply AxiomII in H2; destruct H2, H3.
...
Theorem Theorem126 : forall f x,
  ...
  /\ (forall y:Class, y ∈ dom( f | x) -> (f | x) [y] = f [y]).
Proof.
  ...
  - destruct H0; apply AxiomII in H0; destruct H0 as [_ [H0 _]].
    apply AxiomII in H1; destruct H1 as [_ [H1 _]]; unfold Function in H; eapply H; eauto.
  - apply AxiomI; split; intros.
    + apply AxiomII in H0; destruct H0; destruct H1; apply AxiomII in H1; destruct H1, H2.
        apply Property_dom in H2; apply AxiomII_P in H3; apply AxiomII; split; tauto.
    + apply AxiomII in H0; destruct H0; destruct H1; apply AxiomII; split; auto.
  ...
  - apply AxiomI; split; intros.
    + apply AxiomII in H1; destruct H1.
       apply AxiomII in H0; destruct H0, H3; apply AxiomII in H3; destruct H3, H4.
...
Proof.
  ...
  - destruct H5.
    + right; unfold Included; intros; rewrite Theorem70 in H7; auto; PP H7.
    ...
    + left; unfold Included; intros; rewrite Theorem70 in H7; auto; PP H7.
        double H8; rewrite <- Theorem70 in H8; auto; apply Property_dom in H8.
         apply AxiomII_P in H9; destruct H9; rewrite Theorem70; auto; apply AxiomII_P.
        split; auto; rewrite H10; generalize (classic (f[a] = h[a])); intro; destruct H11; auto.
        assert (a ∈  \{ λ a : Class,a ∈ (dom( f) ∩ dom( h)) /\ f [a] ≠ h [a] \}).
  ...
  - assert (exists u, FirstMember u E \{ λ a : Class,a ∈ (dom( f) ∩ dom( h)) /\ f [a] ≠ h [a] \}).
    ...
    {  apply AxiomI; intros; split; intros.
        + apply AxiomII in H14; destruct H14, H15; apply AxiomII; repeat split; auto; PP H16.
...
Proof.
  ...
  { unfold Function; intros; split; intros.
    + unfold Relation; intros; PP H; eauto.
  ...
  - assert (Ordinal dom( En_f' g)).
    ...
    * apply AxiomII in H2; destruct H2, H3.
    ...
    { apply AxiomI; split; intros; apply AxiomII in H13; destruct H13, H14.
       ...
       PP H15; apply AxiomII_P in H16; apply AxiomII_P; split; auto.
       destruct H16, H17.
    ...
    * generalize H2; intro; apply Theorem69 in H2; auto; rewrite (Lemma128' _ _ H0 H1 H3).
         ...
         + generalize Theorem113; intro; destruct H5 as [H5 _]; apply Theorem107 in H5.
            unfold WellOrdered  in H5; destruct H5.
     ...
     * apply Property_Value in H13; auto; apply AxiomII in H13; destruct H13, H14.
       ...
       { unfold Restriction; rewrite Theorem6'; rewrite Theorem8.
          ...
          { apply AxiomI; split; intros; apply AxiomII in H23; destruct H23, H24; auto.
             PP H24; apply AxiomII_P in H26; destruct H26, H27.
        ...
        -- apply AxiomII in H14; destruct H14.
           ...
           { apply AxiomI; split; intros.
              - apply AxiomII in H10; destruct H10, H18.
                ...
                * apply AxiomII; tauto.
                * PP H19; apply AxiomII_P in H21; destruct H21, H22.
